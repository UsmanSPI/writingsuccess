...
function printDocument(iframe) {
  //  PrintWithIframe()
   ...
   // var win = window.open('');
    mywindow.document.write('<title></title><style>body{margin: 0 auto; text-align: center;}@page { size: auto;  margin: 0mm; }</style><img style="width:100%;height:100%;" src="' +  angularscope.worksheet  + '" onload="window.print();window.close()" />');
    //win.focus();
}
...
/*function PrintWithIframe()
{
    ...
    $('html').append('<iframe id="printf" name="printf" style="width:750px;height:700px;"></iframe>');  // an iFrame is added to the html content, then your div's contents are added to it and the iFrame's content is printed
var mywindow='';
    var mywindow = window.frames["printf"];
    mywindow.document.write('<html><head><title></title><style>@media print{html, body{height:99%;}}body{font-family: "Montserrat", sans-serif !important;}@page{margin: auto}caption{margin-bottom:10px;}table{width:70%;margin:25% auto 40px;border-collapse:collapse!important}table td{position:relative;width:33.33%;border:2px solid #000!important}table td>div{position:absolute;top:0;bottom:0;left:0;right:0;padding:5px}table td>div span{position:absolute;left:0;right:0;top:50%;transform:translateY(-50%);display:block;text-align:center;color:#000;font-size:1.6vmin;font-weight:600;word-wrap:break-word;white-space:pre-line}table td:after{content:"";display:block;margin-top:100%}table.four-column{margin: 25% auto 20px;width:70%}table.four-column td{width:25%}table.five-column{margin: 25% auto 20px;width:75%}table.five-column td{width:20%}table td.logo-preview{border:0!important;width:100%;}table td.logo-preview:after{display:none;}table.isPageBreak{page-break-after: always;}table.twopagelayout{width: 55%;margin: 20px auto 35px; border-collapse: collapse}table.twopagelayout td.logo-preview{position:absolute;right:0;width:100%;height:1px;margin:0;padding:0;border:0;border-collapse: collapse}table.twopagelayout.five-column{width:55.5%;}table.twopagelayout .print-logo{position:absolute;right:-15px;top:-55px;width:207px;height:43px;transform:scale(0.75)}table.twopagelayout.five-column .print-logo{right:-18px;transform:scale(0.7)}.print-words { max-width: 700px; width: 100%; margin: 0 auto; } .print-words ul { margin: 0; padding: 0; column-count: 2; text-align: left; } .print-words ul li { list-style: none; break-inside: avoid-column; padding: 5px; } .print-words ul li button { width:100%; padding: 8px 15px; text-align:left; background-color: #fff; border: 1px solid #b0b0b0; border-radius: 5px; text-transform: capitalize; font-size: 14px; box-shadow: inset 0 0 4px #b4b4b4; }table .print-logo{display:block;width:207px;height:43px;margin:15px auto 0;}</style>'  // Your styles here, I needed the margins set up like this
        + '</head><body style="text-align:center;"><div><img src="' +  angularscope.worksheet  + '" onload="window.print()"/>'
        + '</div></body></html>');
// $(mywindow.document).find('img').on('load',function(){
	// mywindow.print();
// });
...
function drawpoints() {
    ...
    //    guideimageicon.style.width="40px";
    //  guideimageicon.style.height="40px";
